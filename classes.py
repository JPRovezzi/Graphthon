...
class experiment:
    def __init__(self, system, temperature_series=None, time_series=None):
        self.system = system
        self.temperature_series = temperature_series if temperature_series is not None else []
        self.time_series = time_series if time_series is not None else []

    def get_temperature_series(self):
        return self.temperature_series
    ...
    def get_system_components(self):
        return {
            "alcohol": self.system.alcohol,
            "acid": self.system.acid,
            "catalyst": self.system.catalyst
        }

    def get_time_offset(self):
        if not self.time_series or len(self.time_series) < 2:
            return 0
        return [self.time_series[i+1] - self.time_series[i] for i in range(len(self.time_series)-1)]


class tititation(experiment):
    def __init__(self, system, temperature_series=None, time_series=None, sample_names=None):
        super().__init__(system, temperature_series, time_series)
        self.sample_names = sample_names if sample_names is not None else []

    def get_sample_names(self):
        return self.sample_names

    def add_sample(self, name, time, temperature):
        self.sample_names.append(name)
        self.time_series.append(time)
        self.temperature_series.append(temperature)

    def get_sample_info(self, idx):
        if idx < 0 or idx >= self.get_num_samples():
            raise IndexError("Sample index out of range.")
        return {
            "name": self.sample_names[idx] if idx < len(self.sample_names) else None,
            "time": self.time_series[idx],
            "temperature": self.temperature_series[idx] if idx < len(self.temperature_series) else None
        }
